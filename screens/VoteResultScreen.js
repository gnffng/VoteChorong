...
const VoteResultScreen = ({ navigation, route }) => {
    ...
    <SafeAreaView style={[{flex:1}]}>
      ...
      <View style={[styles.verticalLayout,{flex:1, backgroundColor:baseColor}]}>
          ...
          <View style={[{flex:10}]}>
            <View style={[{flex:1}]} />
            <TouchableOpacity 
              style={[{flex:10, borderRadius:10, backgroundColor:'#CC0', borderColor:'#DD0', borderWidth:3}]}
              onPress={() => { 
                StorageManager.readIdGenerator().then((idGenerator) => {
                  StorageManager.readData().then((data) => {
                    StorageManager.storeData([...data, {
                      ...
                      StorageManager.storeIdGenerator(""+(Number(idGenerator)+1)).then(() => {
                        setIsInit(true);
                        navigation.navigate("MAIN");
                      });
                    })
            ...
            <Text style={[{flex:1, color:'white', fontSize:Theme.fontSizes.fontSizes22, textAlign:'center', textAlignVertical:'center'}]}>결과 저장</Text>
            </TouchableOpacity>
            <View style={[{flex:1}]} />
          </View>
          <View style={[{flex:1}]} />
